// ignore: unused_import
import 'package:intl/intl.dart' as intl;
import 'app_localizations.dart';

// ignore_for_file: type=lint

/// The translations for Russian (`ru`).
class AppLocalizationsRu extends AppLocalizations {
  AppLocalizationsRu([String locale = 'ru']) : super(locale);

  @override
  String get app_name => 'TISM';

  @override
  String get app_subtitle => 'Всё, что нужно знать об РАС в одном клике';

  @override
  String get app_title => 'TISM - Руководство по РАС';

  @override
  String get login => 'Войти';

  @override
  String get register => 'Регистрация';

  @override
  String get email => 'Email';

  @override
  String get password => 'Пароль';

  @override
  String get confirm_password => 'Подтвердить пароль';

  @override
  String get name => 'Имя';

  @override
  String get username => 'Имя пользователя';

  @override
  String get forgot_password => 'Забыл пароль';

  @override
  String get login_success => 'Вход выполнен успешно';

  @override
  String get register_success => 'Аккаунт создан успешно';

  @override
  String get logout => 'Выйти';

  @override
  String get logout_confirm => 'Вы уверены, что хотите выйти?';

  @override
  String get participant => 'Участник';

  @override
  String get participant_desc => 'Обычный пользователь TISM';

  @override
  String get responsible => 'Ответственный';

  @override
  String get responsible_desc => 'Член семьи или опекун';

  @override
  String get professional => 'Специалист';

  @override
  String get professional_desc => 'Терапевт, врач, педагог';

  @override
  String get user_type => 'Тип пользователя';

  @override
  String get select_user_type => 'Выберите ваш тип';

  @override
  String get profile => 'Профиль';

  @override
  String get edit_profile => 'Редактировать Профиль';

  @override
  String get edit_name => 'Редактировать Имя';

  @override
  String get edit_username => 'Редактировать Имя пользователя';

  @override
  String get full_name => 'Полное имя';

  @override
  String get account_type => 'Тип аккаунта';

  @override
  String get theme => 'Тема';

  @override
  String get light_theme => 'Светлая';

  @override
  String get dark_theme => 'Тёмная';

  @override
  String get system_theme => 'Системная';

  @override
  String get system_theme_desc => 'Следует теме устройства';

  @override
  String get home => 'Главная';

  @override
  String get forum => 'Форум';

  @override
  String get knowledge => 'Знания';

  @override
  String get routine => 'Распорядок';

  @override
  String get dashboard => 'Панель';

  @override
  String get feed => 'Лента';

  @override
  String get search => 'Поиск';

  @override
  String get hello => 'Привет';

  @override
  String hello_user(String name) {
    return 'Привет, $name! 👋';
  }

  @override
  String get explore_content => 'Изучайте образовательный контент о РАС';

  @override
  String get educational_feed => 'Образовательная Лента';

  @override
  String get personalized_routine => 'Персонализированный Распорядок';

  @override
  String get custom_routine => 'Персональный Распорядок';

  @override
  String get observation_diary => 'Дневник Наблюдений';

  @override
  String get tina_chatbot => 'Тина (Чатбот)';

  @override
  String get tea_forum => 'Форум РАС';

  @override
  String get articles => 'Статьи';

  @override
  String get digital_library => 'Цифровая Библиотека';

  @override
  String get new_post => 'Новый Пост';

  @override
  String get publish => 'Опубликовать';

  @override
  String get share_experience =>
      'Поделитесь своим опытом, вопросом или советом о РАС...';

  @override
  String get write_something => 'Напишите что-нибудь перед публикацией';

  @override
  String get confirm_publication => 'Подтвердить Публикацию';

  @override
  String get want_to_publish => 'Хотите опубликовать этот пост на форуме?';

  @override
  String get post_published => 'Пост опубликован успешно!';

  @override
  String get error_publishing => 'Ошибка публикации';

  @override
  String get tina_assistant => 'Тина - Помощник по РАС';

  @override
  String get thinking => 'Думаю...';

  @override
  String get type_message => 'Введите ваше сообщение...';

  @override
  String get about_tina => '💙 О Тине';

  @override
  String get tina_intro =>
      '🤖 **Привет! Я Тина!**\n\nЯ виртуальный помощник, специализирующийся на аутизме и нейроразнообразии, разработанный специально для TISM мультидисциплинарной командой экспертов.';

  @override
  String get tina_specialization =>
      '🎯 **Моя специализация:**\n• 🧠 Развитие и поведение\n• 🏫 Инклюзивные образовательные стратегии\n• 💬 Техники коммуникации и социального взаимодействия\n• 🌍 Экологические и сенсорные адаптации\n• 🛠️ Практические ресурсы для повседневной жизни';

  @override
  String get tina_scientific =>
      '🔬 **Научная основа:** Я была обучена знаниями, подтверждёнными неврологами, психологами, эрготерапевтами, логопедами и специальными педагогами.';

  @override
  String get tina_important =>
      '⚠️ **Важно:** Я не ставлю диагнозы и не заменяю медицинских специалистов. Моя роль дополнительная, предлагающая информационную и практическую поддержку.';

  @override
  String get tina_support =>
      '💙 **Я здесь, чтобы поддержать вас надёжной информацией и эмпатией!**';

  @override
  String get understood_tina => 'Понятно, Тина! Давайте поговорим! 😊';

  @override
  String get tina_welcome =>
      'Привет! Я Тина, виртуальный помощник, специализирующийся на аутизме и нейроразнообразии от TISM!\n\nЯ здесь, чтобы предложить персонализированную поддержку и информацию, основанную на научных данных о:\n\n• Развитии и поведении\n• Инклюзивных образовательных стратегиях\n• Техниках коммуникации и социального взаимодействия\n• Экологических и сенсорных адаптациях\n• Практических ресурсах и инструментах для повседневной жизни\n\nМоя база знаний была разработана мультидисциплинарной командой экспертов, включая неврологов, психологов, эрготерапевтов, логопедов и специальных педагогов.\n\nВажно отметить, что я не ставлю диагнозы и не заменяю медицинских специалистов - моя роль дополнительная, предлагающая надёжную информацию и практическую поддержку для семей, опекунов и нейроразнообразных людей.\n\nКак я могу помочь вам сегодня?';

  @override
  String get tina_connection_error =>
      'У меня сейчас проблемы с подключением. Много пользователей используют систему. Попробуйте через несколько минут! 😅';

  @override
  String get tina_general_error =>
      'Ой! Что-то пошло не так. Попробуйте через несколько мгновений! 🤖';

  @override
  String get tina_api_key_error =>
      'Неверный API-ключ. Проверьте, что ключ Gemini корректен в файле .env';

  @override
  String get tina_quota_error =>
      'Превышен лимит использования API. Попробуйте позже.';

  @override
  String get tina_network_error =>
      'Ошибка соединения. Проверьте интернет и попробуйте снова.';

  @override
  String get diary_observations => 'Дневник Наблюдений';

  @override
  String get no_observations => 'Пока нет наблюдений';

  @override
  String get tap_plus_start => 'Нажмите + чтобы начать';

  @override
  String get report_copied => 'Отчёт скопирован! Вставьте в email или WhatsApp';

  @override
  String get progress => 'Прогресс';

  @override
  String get behavior => 'Поведение';

  @override
  String get crisis => 'Кризис';

  @override
  String get difficulty => 'Трудность';

  @override
  String get triggers => 'Триггеры';

  @override
  String routine_of(String name) {
    return 'Распорядок $name';
  }

  @override
  String support_level(String level) {
    return 'Поддержка $level';
  }

  @override
  String get filter_category => 'Фильтр по категории:';

  @override
  String get completed => 'Завершённые';

  @override
  String progress_count(int completed, int total) {
    return 'Прогресс: $completed/$total';
  }

  @override
  String get no_activities => 'Активности не найдены';

  @override
  String get clear_filters => 'Очистить фильтры';

  @override
  String get add_activity => 'Добавить Активность';

  @override
  String get edit_activity => 'Редактировать Активность';

  @override
  String get morning => 'Утро';

  @override
  String get education => 'Образование';

  @override
  String get food => 'Питание';

  @override
  String get leisure => 'Досуг';

  @override
  String get wellness => 'Благополучие';

  @override
  String get night => 'Ночь';

  @override
  String get all => 'Все';

  @override
  String get welcome => 'Привет, добро пожаловать!';

  @override
  String get profile_setup_desc =>
      'Чтобы создать персонализированный распорядок, давайте сначала настроим профиль ребёнка.\n\nЭто поможет нам предложить подходящие по возрасту активности и уровню поддержки.';

  @override
  String get create_profile => 'Создать Профиль';

  @override
  String get edit_profile_menu => 'Редактировать Профиль';

  @override
  String get switch_child => 'Сменить Ребёнка';

  @override
  String get new_child => 'Новый Ребёнок';

  @override
  String get select_child => 'Выбрать Ребёнка';

  @override
  String get verify_email => 'Подтвердить Email';

  @override
  String get verification_sent => 'Email подтверждения отправлен!';

  @override
  String get check_email => 'Проверьте вашу почту';

  @override
  String get verification_desc =>
      'Нажмите на ссылку в email, чтобы активировать ваш аккаунт.\nВы будете перенаправлены автоматически.';

  @override
  String get already_verified => 'Уже подтверждён - Войти';

  @override
  String get resend_verification => 'Отправить email подтверждения повторно';

  @override
  String get verifying => 'Проверяю...';

  @override
  String get delete_account => 'Удалить Аккаунт';

  @override
  String get delete_warning =>
      'Это действие НЕОБРАТИМО!\n\nВсе ваши данные будут потеряны навсегда.';

  @override
  String get delete_confirmation => 'Для подтверждения введите точно:';

  @override
  String get delete_phrase => 'УДАЛИТЬ мой аккаунт';

  @override
  String get delete_input_hint => 'Введите фразу выше';

  @override
  String get delete_incorrect =>
      'Неправильная фраза. Проверьте заглавные и строчные буквы.';

  @override
  String get confirm_password_delete => 'Подтвердить пароль';

  @override
  String get enter_password => 'Введите ваш пароль';

  @override
  String get save => 'Сохранить';

  @override
  String get cancel => 'Отменить';

  @override
  String get confirm => 'Подтвердить';

  @override
  String get delete => 'Удалить';

  @override
  String get edit => 'Редактировать';

  @override
  String get add => 'Добавить';

  @override
  String get remove => 'Удалить';

  @override
  String get update => 'Обновить';

  @override
  String get close => 'Закрыть';

  @override
  String get ok => 'OK';

  @override
  String get yes => 'Да';

  @override
  String get no => 'Нет';

  @override
  String get share => 'Поделиться';

  @override
  String get success => 'Успех';

  @override
  String get error => 'Ошибка';

  @override
  String get loading => 'Загрузка...';

  @override
  String get connection_error => 'Ошибка соединения';

  @override
  String get try_again => 'Попробовать снова';

  @override
  String get updated_successfully => 'Обновлено успешно!';

  @override
  String get deleted_successfully => 'Удалено успешно';

  @override
  String get saved_successfully => 'Сохранено успешно!';

  @override
  String get welcome_message => 'Привет, добро пожаловать!';

  @override
  String get no_account => 'Ещё нет аккаунта? Зарегистрируйтесь';

  @override
  String get fill_all_fields => 'Заполните все поля';

  @override
  String get login_error => 'Ошибка входа';

  @override
  String connection_error_detail(String error) {
    return 'Ошибка соединения: $error';
  }

  @override
  String get field_required => 'Это поле обязательно';

  @override
  String get invalid_email => 'Неверный email';

  @override
  String get password_too_short => 'Пароль должен содержать минимум 8 символов';

  @override
  String get passwords_dont_match => 'Пароли не совпадают';

  @override
  String get username_invalid =>
      'Имя пользователя должно содержать только строчные буквы, цифры и _';

  @override
  String get name_cooldown => 'Имя можно изменить только раз в день';

  @override
  String get username_cooldown =>
      'Имя пользователя можно изменить только раз в 3 дня';

  @override
  String get user_type_cooldown =>
      'Тип пользователя можно изменить только раз в день';

  @override
  String get wake_up => 'Просыпаться';

  @override
  String get wake_up_desc => 'Просыпаться под мягкую музыку';

  @override
  String get brush_teeth => 'Чистить зубы';

  @override
  String get brush_teeth_desc => 'Использовать мягкую щётку 2 минуты';

  @override
  String get breakfast => 'Завтрак';

  @override
  String get breakfast_desc => 'Есть сидя за столом';

  @override
  String get educational_activity => 'Образовательная активность';

  @override
  String get educational_activity_desc => 'Образовательные игры или школа';

  @override
  String get snack => 'Перекус';

  @override
  String get snack_desc => 'Фрукты или здоровый перекус';

  @override
  String get music => 'Музыка';

  @override
  String get music_desc => 'Слушать или играть музыку';

  @override
  String get draw => 'Рисовать';

  @override
  String get draw_desc => 'Свободное рисование';

  @override
  String get numbers => 'Числа';

  @override
  String get numbers_desc => 'Игры с числами';

  @override
  String get sensory_break => 'Сенсорная пауза';

  @override
  String get sensory_break_desc => 'Момент спокойствия и саморегуляции';

  @override
  String get dinner => 'Ужин';

  @override
  String get dinner_desc => 'Семейная трапеза';

  @override
  String get bath => 'Ванна';

  @override
  String get bath_desc => 'Тёплая вода, мягкая губка';

  @override
  String get sleep => 'Спать';

  @override
  String get sleep_desc => 'Мягкая музыка для сна';

  @override
  String get welcome_forum => 'Добро пожаловать на Форум РАС! 💙';

  @override
  String get first_post_message =>
      'Будьте первым, кто поделится опытом\nили задаст вопрос сообществу';

  @override
  String get create_first_post => 'Создать первый пост';

  @override
  String get delete_post => 'Удалить Пост';

  @override
  String get delete_post_confirm =>
      'Вы уверены, что хотите удалить этот пост? Это действие нельзя отменить.';

  @override
  String get post_deleted_success => 'Пост удалён успешно';

  @override
  String error_deleting_post(String error) {
    return 'Ошибка удаления поста: $error';
  }

  @override
  String get report_post => 'Пожаловаться';

  @override
  String get report_post_title => 'Пожаловаться на Пост';

  @override
  String get report_post_confirm =>
      'Вы уверены, что хотите пожаловаться на этот пост?';

  @override
  String get post_reported => 'Жалоба на пост отправлена успешно';

  @override
  String get now => 'Сейчас';

  @override
  String get search_posts => 'Искать посты, людей или темы...';

  @override
  String get no_results_found => 'Результаты не найдены';

  @override
  String get try_different_keywords =>
      'Попробуйте использовать другие ключевые слова\nили изучите категории ниже';

  @override
  String get type_to_search => 'Введите для поиска...';

  @override
  String get find_posts_profiles =>
      'Найдите посты, профили и хештеги\nв сообществе РАС';

  @override
  String get how_identify_autism => 'Как определить аутизм?';

  @override
  String get child_not_speaking => 'Мой ребёнок не говорит, это аутизм?';

  @override
  String get what_therapies_work => 'Какие терапии работают?';

  @override
  String get help_at_school => 'Как помочь в школе?';

  @override
  String get autism_rights => 'Права аутистов';

  @override
  String get child_tantrums => 'Ребёнок с истериками';

  @override
  String get new_observation => 'Новое Наблюдение';

  @override
  String get edit_observation => 'Редактировать Наблюдение';

  @override
  String get observation_type => 'Тип:';

  @override
  String get title => 'Заголовок';

  @override
  String get detailed_description => 'Подробное описание';

  @override
  String intensity(int level) {
    return 'Интенсивность: $level';
  }

  @override
  String get observer => 'Наблюдатель:';

  @override
  String get possible_triggers => 'Возможные триггеры:';

  @override
  String get delete_observation => 'Удалить Наблюдение';

  @override
  String get delete_observation_confirm =>
      'Вы уверены, что хотите удалить это наблюдение?';

  @override
  String get observation_title_empty =>
      'Заголовок наблюдения не может быть пустым';

  @override
  String get not_defined => 'Не определено';

  @override
  String get confirm_change => 'Подтвердить изменение';

  @override
  String change_name_to(String name) {
    return 'Изменить имя на \"$name\"?';
  }

  @override
  String change_username_to(String oldName, String newName) {
    return 'Изменить имя пользователя с @$oldName на @$newName?';
  }

  @override
  String get error_updating_name => 'Ошибка обновления имени';

  @override
  String get error_updating_username => 'Ошибка обновления имени пользователя';

  @override
  String get error_updating_type => 'Ошибка обновления типа';

  @override
  String get username_help => 'Только строчные буквы, цифры и _';

  @override
  String get username_cooldown_info =>
      'Имя пользователя можно изменить только раз в 3 дня';

  @override
  String get confirm_password_title => 'Подтвердить пароль';

  @override
  String get error_deleting_account => 'Ошибка удаления аккаунта';

  @override
  String get type_updated => 'Тип обновлён!';

  @override
  String get name_updated => 'Имя обновлено!';

  @override
  String get username_updated => 'Имя пользователя обновлено!';

  @override
  String get account_deleted => 'Аккаунт удалён успешно';

  @override
  String get incorrect_password => 'Неверный пароль';

  @override
  String get user_not_found => 'Пользователь не найден';

  @override
  String get email_not_verified => 'Email ещё не подтверждён';

  @override
  String get verification_email_sent =>
      'Email подтверждения отправлен повторно!';

  @override
  String get checking_verification => 'Проверяю...';

  @override
  String get already_have_account => 'Уже есть аккаунт? Войти';

  @override
  String get create_account => 'Создать аккаунт';

  @override
  String get account_created => 'Аккаунт создан! Проверьте email для активации';

  @override
  String get example123 => 'пример123';

  @override
  String get new_routine => 'Новый Распорядок';

  @override
  String get edit_routine => 'Редактировать Распорядок';

  @override
  String get activity_title => 'Название активности';

  @override
  String get activity_description => 'Описание активности';

  @override
  String get time_format => 'Время (00:00 - 23:59)';

  @override
  String get category => 'Категория';

  @override
  String get icon => 'Иконка';

  @override
  String get color => 'Цвет';

  @override
  String get create_routine => 'Создать Распорядок';

  @override
  String get save_changes => 'Сохранить Изменения';

  @override
  String get routine_title_empty => 'Название распорядка не может быть пустым';

  @override
  String get invalid_time_format =>
      'Неверное время. Используйте 24-часовой формат (00:00 - 23:59)';

  @override
  String get delete_routine => 'Удалить Распорядок';

  @override
  String get delete_routine_confirm =>
      'Вы уверены, что хотите удалить этот распорядок?';

  @override
  String hello_user_name(Object name) {
    return 'Привет, $name! 👋';
  }

  @override
  String get statistics => 'Статистика';

  @override
  String get general_summary => 'Общая Сводка';

  @override
  String get total => 'Всего';

  @override
  String get progress_plural => 'Прогресс';

  @override
  String get crisis_plural => 'Кризисы';

  @override
  String get type_distribution => 'Распределение по Типам';

  @override
  String get most_frequent_triggers => 'Наиболее Частые Триггеры';

  @override
  String get weekly_trend => 'Недельная Тенденция';

  @override
  String observations_last_7_days(int count) {
    return 'Наблюдения за последние 7 дней: $count';
  }

  @override
  String get child_name => 'Имя ребёнка';

  @override
  String get age => 'Возраст';

  @override
  String get support_level_label => 'Уровень поддержки';

  @override
  String get sensory_preferences => 'Сенсорные предпочтения';

  @override
  String get interests => 'Интересы';

  @override
  String get child_name_empty => 'Имя ребёнка не может быть пустым';

  @override
  String observations_last_7_days_count(int count) {
    return 'Наблюдения за последние 7 дней: $count';
  }

  @override
  String get configure_profile => 'Настроить Профиль';

  @override
  String get edit_username_title => 'Редактировать Имя пользователя';

  @override
  String get account_deleted_success => 'Аккаунт удалён успешно';

  @override
  String get theme_light => 'Светлая';

  @override
  String get theme_dark => 'Тёмная';

  @override
  String get theme_system => 'Системная';

  @override
  String get edit_observation_action => 'Редактировать наблюдение';

  @override
  String get delete_observation_action => 'Удалить наблюдение';

  @override
  String get delete_observation_title => 'Удалить Наблюдение';

  @override
  String get delete_observation_message =>
      'Вы уверены, что хотите удалить это наблюдение?';

  @override
  String get trigger_routine_change => 'Изменение распорядка';

  @override
  String get trigger_loud_noise => 'Громкий шум';

  @override
  String get trigger_crowd => 'Толпа';

  @override
  String get trigger_tiredness => 'Усталость';

  @override
  String get trigger_hunger => 'Голод';

  @override
  String get trigger_frustration => 'Фрустрация';

  @override
  String get trigger_transition => 'Переход';

  @override
  String get trigger_new_environment => 'Новая среда';

  @override
  String get trigger_bright_light => 'Яркий свет';

  @override
  String get trigger_unpleasant_texture => 'Неприятная текстура';

  @override
  String get trigger_strong_smell => 'Сильный запах';

  @override
  String get trigger_temperature => 'Температура';

  @override
  String get trigger_tight_clothes => 'Тесная одежда';

  @override
  String get trigger_insufficient_sleep => 'Недостаточный сон';

  @override
  String get trigger_physical_pain => 'Физическая боль';

  @override
  String get trigger_medication => 'Медикаменты';

  @override
  String get trigger_medical_visit => 'Визит к врачу';

  @override
  String get trigger_new_school => 'Новая школа';

  @override
  String get trigger_substitute_teacher => 'Учитель-заместитель';

  @override
  String get trigger_test_evaluation => 'Тест/оценка';

  @override
  String get trigger_party_event => 'Вечеринка/событие';

  @override
  String get trigger_travel => 'Путешествие';

  @override
  String get trigger_rain_storm => 'Дождь/буря';

  @override
  String get trigger_parents_separation => 'Разлука родителей';

  @override
  String get trigger_broken_toy => 'Сломанная игрушка';

  @override
  String get trigger_not_getting_something => 'Не получить что-то';

  @override
  String get trigger_activity_interruption => 'Прерывание активности';

  @override
  String get trigger_waiting_too_long => 'Слишком долгое ожидание';

  @override
  String get observer_father => 'отец';

  @override
  String get observer_mother => 'мать';

  @override
  String get observer_grandfather => 'дедушка';

  @override
  String get observer_grandmother => 'бабушка';

  @override
  String get observer_uncle => 'дядя';

  @override
  String get observer_aunt => 'тётя';

  @override
  String get observer_brother => 'брат';

  @override
  String get observer_sister => 'сестра';

  @override
  String get observer_son => 'сын';

  @override
  String get observer_daughter => 'дочь';

  @override
  String get observer_grandson => 'внук';

  @override
  String get observer_granddaughter => 'внучка';

  @override
  String get observer_nephew => 'племянник';

  @override
  String get observer_niece => 'племянница';

  @override
  String get observer_male_cousin => 'двоюродный брат';

  @override
  String get observer_female_cousin => 'двоюродная сестра';

  @override
  String get observer_male_friend => 'друг';

  @override
  String get observer_female_friend => 'подруга';

  @override
  String get observer_relative => 'родственник';

  @override
  String get observer_caregiver => 'опекун';

  @override
  String get observer_teacher => 'учитель';

  @override
  String get observer_therapist => 'терапевт';

  @override
  String get observer_doctor => 'врач';

  @override
  String get observer_psychologist => 'психолог';

  @override
  String get support_level_mild => 'лёгкий';

  @override
  String get support_level_moderate => 'умеренный';

  @override
  String get support_level_severe => 'тяжёлый';

  @override
  String get sensory_visual => 'Визуальный';

  @override
  String get sensory_auditory => 'Слуховой';

  @override
  String get sensory_tactile => 'Тактильный';

  @override
  String get sensory_movement => 'Движение';

  @override
  String get sensory_olfactory => 'Обонятельный';

  @override
  String get sensory_gustatory => 'Вкусовой';

  @override
  String get sensory_proprioceptive => 'Проприоцептивный';

  @override
  String get sensory_vestibular => 'Вестибулярный';

  @override
  String get sensory_deep_pressure => 'Глубокое Давление';

  @override
  String get sensory_soft_textures => 'Мягкие Текстуры';

  @override
  String get sensory_rough_textures => 'Грубые Текстуры';

  @override
  String get sensory_low_sounds => 'Тихие Звуки';

  @override
  String get sensory_high_sounds => 'Громкие Звуки';

  @override
  String get sensory_soft_lights => 'Мягкий Свет';

  @override
  String get sensory_bright_lights => 'Яркий Свет';

  @override
  String get sensory_hot_temperatures => 'Горячие Температуры';

  @override
  String get sensory_cold_temperatures => 'Холодные Температуры';

  @override
  String get interest_music => 'Музыка';

  @override
  String get interest_drawing => 'Рисование';

  @override
  String get interest_numbers => 'Числа';

  @override
  String get interest_animals => 'Животные';

  @override
  String get interest_cars => 'Машины';

  @override
  String get interest_books => 'Книги';

  @override
  String get interest_games => 'Игры';

  @override
  String get interest_computer => 'Компьютер';

  @override
  String get interest_tablet => 'Планшет';

  @override
  String get interest_toys => 'Игрушки';

  @override
  String get interest_sports => 'Спорт';

  @override
  String get interest_dance => 'Танцы';

  @override
  String get interest_cooking => 'Готовка';

  @override
  String get interest_gardening => 'Садоводство';

  @override
  String get interest_science => 'Наука';

  @override
  String get interest_math => 'Математика';

  @override
  String get interest_art => 'Искусство';

  @override
  String get interest_photography => 'Фотография';

  @override
  String get interest_videos => 'Видео';

  @override
  String get interest_movies => 'Фильмы';

  @override
  String get interest_series => 'Сериалы';

  @override
  String get interest_puzzles => 'Пазлы';

  @override
  String get interest_lego => 'Лего';

  @override
  String get interest_dolls => 'Куклы';

  @override
  String get interest_superheroes => 'Супергерои';

  @override
  String get delete_post_action => 'Удалить';

  @override
  String get report_post_action => 'Пожаловаться';

  @override
  String get clear_search => 'Очистить поиск';

  @override
  String get month => 'месяц';

  @override
  String get months => 'месяцев';

  @override
  String get year => 'год';

  @override
  String get years => 'лет';

  @override
  String get search_articles => 'Поиск статей...';

  @override
  String get all_categories => 'Все категории';

  @override
  String get no_articles_found => 'Статьи не найдены';

  @override
  String get no_articles_available => 'Нет доступных статей';

  @override
  String get article_early_signs_title =>
      'Ранние признаки РАС: на что обращать внимание';

  @override
  String get article_early_signs_body =>
      'Ранняя диагностика расстройств аутистического спектра (ASD) крайне важна для развития ребёнка. Первые признаки могут появиться в первые месяцы жизни. Основные индикаторы включают: трудности в социальном общении, такие как избегание зрительного контакта, отсутствие реакции на имя или неуказывание на интересные объекты; повторяющиеся поведенческие паттерны, такие как стереотипные движения рук или сосредоточенность на определённых объектах; сенсорные чувствительности, чрезмерная или недостаточная реакция на звуки, прикосновения или свет; задержка языкового развития, включая отсутствие лепета в 12 месяцев или слов в 16 месяцев. Важно помнить, что каждый ребёнок уникален и может проявлять различные комбинации этих признаков. Если вы замечаете некоторые из этих поведений, обратитесь за руководством к квалифицированному специалисту для получения соответствующей оценки.';

  @override
  String get article_early_signs_author => 'Др. Анна Сильва - детский невролог';

  @override
  String get article_aba_therapy_title =>
      'Прикладной анализ поведения (ABA): принципы и преимущества';

  @override
  String get article_aba_therapy_body =>
      'Прикладной анализ поведения (ABA) является одним из наиболее изученных и эффективных вмешательств для людей с аутизмом. Основанный на научных принципах поведения, ABA использует систематические методы для обучения навыкам и снижения проблемного поведения. Основные компоненты включают: функциональная оценка поведения для определения причин поведения; структурированное обучение социальным, коммуникативным и академическим навыкам; использование положительного подкрепления для мотивации и поддержания желаемого поведения; сбор данных для мониторинга прогресса; обобщение приобретённых навыков на различные среды. Лечение ABA должно быть индивидуализированным, учитывая специфические потребности каждого человека. При применении квалифицированными специалистами оно может привести к значительным улучшениям в области коммуникации, социального взаимодействия, адаптивного поведения и общего качества жизни.';

  @override
  String get article_aba_therapy_author =>
      'Др. Мария Сантос - аналитик поведения';

  @override
  String get article_routines_title =>
      'Важность распорядка для людей с аутизмом';

  @override
  String get article_routines_body =>
      'Структурированный распорядок является основополагающим для благополучия людей с аутизмом, обеспечивая предсказуемость и безопасность в мире, который может казаться хаотичным. Преимущества распорядка включают: снижение тревоги через предсказуемость; развитие независимости и автономии; улучшение временной и пространственной организации; облегчение изучения новых навыков; снижение сложного поведения. Для внедрения эффективного распорядка: используйте визуальную поддержку, такую как календари и расписания; поддерживайте последовательность во времени и деятельности; готовьте человека к изменениям заранее; включайте приятные деятельности в распорядок; будьте гибкими при необходимости и вносите изменения постепенно. Помните, что у каждого человека есть свои потребности и предпочтения. Цель состоит в том, чтобы создать структуру, которая способствует развитию и благополучию, а не жёсткость, вызывающую стресс.';

  @override
  String get article_routines_author => 'Проф. Жоао Оливейра - эрготерапевт';

  @override
  String get article_school_inclusion_title =>
      'Школьная инклюзия: стратегии успеха';

  @override
  String get article_school_inclusion_body =>
      'Школьная инклюзия для учащихся с аутизмом требует тщательного планирования и специфических стратегий для обеспечения академического и социального успеха. Ключевые элементы включают: структурированная среда с ясным и предсказуемым распорядком; куррикулярные адаптации, уважающие стиль обучения учащегося; использование визуальной и технологической поддержки; развитие социальных навыков через направляемые деятельности; сотрудничество между семьёй, школой и специалистами; непрерывное обучение учителей по вопросам аутизма. Практические стратегии: создание тихого пространства для моментов саморегуляции; использование визуальных расписаний для ежедневных деятельностей; внедрение сенсорных пауз при необходимости; поощрение позитивных социальных взаимодействий со сверстниками; создание альтернативных систем коммуникации при необходимости. Успех инклюзии зависит от приверженности всего школьного сообщества созданию доброжелательной среды, адаптированной к индивидуальным потребностям каждого учащегося.';

  @override
  String get article_school_inclusion_author =>
      'Др. Карла Мендес - педагогический психолог';

  @override
  String get language => 'Idioma';

  @override
  String get article_sensory_processing_title => 'Сенсорная обработка при РАС';

  @override
  String get article_sensory_processing_body =>
      'Многие люди с РАС имеют различия в сенсорной обработке, будучи либо гиперчувствительными, либо гипочувствительными к стимулам окружающей среды.';

  @override
  String get article_sensory_processing_author =>
      'Др. Фернанда Роша - эрготерапевт';

  @override
  String get article_family_support_title =>
      'Семейная поддержка: забота о тех, кто заботится';

  @override
  String get article_family_support_body =>
      'Семьи людей с РАС сталкиваются с уникальными вызовами и нуждаются в постоянной поддержке. Самообслуживание опекунов необходимо для благополучия всей семьи.';

  @override
  String get article_family_support_author =>
      'Др. Беатрис Альмейда - семейный психолог';

  @override
  String get article_communication_strategies_title =>
      'Эффективные стратегии общения';

  @override
  String get article_communication_strategies_body =>
      'Общение с людьми в спектре аутизма может быть облегчено через специфические стратегии, которые уважают их потребности и предпочтения.';

  @override
  String get article_communication_strategies_author =>
      'Др. Патрисия Мендес - логопед';
}
